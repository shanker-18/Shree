...
const CategoryPage: React.FC = () => {
  ...
  const [modalWeights, setModalWeights] = useState<('100g'|'500g'|'1kg')[]>(['500g']);
  const [modalQuantities, setModalQuantities] = useState<{[key: string]: number}>({ '100g': 1, '500g': 1, '1kg': 1 });
  ...
  const handleQuickView = (productName: string) => {
    ...
    if (isPuliyo || isVathak || isTurmeric || isPoondu || isAndra) {
      ...
      // Health Mix: allow 250g and 1kg (no 100g/500g)
      setModalWeights(['250g']);
    } else if (isCoffee) {
      // Coffee powder: allow 500g and 1kg (remove 100g); default to 500g
      setModalWeights(['500g']);
    } else {
      setModalWeights(allowed.length ? [allowed[0]] : ['500g']);
    }
    setModalQuantities({ '100g': 1, '500g': 1, '1kg': 1 });
  ...
  const handleModalBuyNow = () => {
    if (modalProduct) {
      ...
      const totalQuantity = weights.reduce((sum, w) => sum + (modalQuantities[w] || 1), 0);
      const productTotal = modalProduct.price * totalQuantity;
      let discountAmount = 0;
      let finalAmount = productTotal;
      // Note: Free samples discount logic removed - only add samples if user explicitly selects them
  ...
  const handleModalAddToCart = () => {
    if (modalProduct) {
      const weights = modalWeights.length ? modalWeights : ['500g'];
      weights.forEach(w => addToCart({ product_name: `${modalProduct.name} (${w})`, category: modalProduct.category, price: modalProduct.price, quantity: modalQuantities[w] || 1 }));
    }
  };
            ...
            <div className="p-4 sm:p-6">
              <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="bg-gray-50 rounded-xl p-4 sm:p-6 flex items-center justify-center h-60 sm:h-80">
                  {modalProduct.image ? (
                    (() => {
                      const sel = (modalWeights[modalWeights.length-1] || '500g') as ('100g'|'500g'|'1kg');
                    ...
                    <div className="flex gap-2">
                      {(() => {
                        ...
                        if (tl.includes('health mix') || tl.includes('healthmix')) {
                          // Exactly 250g and 1kg
                          weights = ['250g','1kg'];
                        } else if (tl.includes('coffee') && tl.includes('powder')) {
                          // Coffee powder: 500g and 1kg only
                          weights = ['500g','1kg'];
                        } else {
                          weights = (
