...
const FirstOrderDiscountPopup: React.FC<FirstOrderDiscountPopupProps> = ({ isVisible, onClose, onClaim }) => {
  ...
  // Handle claim button click - always take user to categories on homepage
  const handleClaimClick = async () => {
    setClaiming(true);
 
    try {
      // Close popup and navigate to homepage categories section
      onClose();
      navigate('/', { state: { scrollTo: 'categories' } });
      onClaim();
    } catch (error) {
      console.error('Error handling discount navigation:', error);
    } finally {
      setClaiming(false);
    }
  };
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="bg-gradient-to-r from-red-500 via-purple-500 to-pink-500 px-6 py-6 text-center text-white relative overflow-hidden">
          ...
          <h2 className="text-2xl font-bold mb-1">15% OFF</h2>
          <p className="text-white text-opacity-90 font-medium text-sm">On orders ₹500+</p>
        </div>
        ...
        <div className="p-6">
          ...
          <p className="text-gray-600 text-center mb-4 text-sm leading-relaxed">
            Place an order for ₹500 or more and get <span className="font-bold text-red-600">15% off</span> on the total.
          </p>
          ...
          <div className="bg-gradient-to-r from-red-50 to-pink-50 rounded-lg p-3 mb-4 border border-red-100">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <h4 className="font-semibold text-red-800 text-sm mb-1">Ongoing Offer</h4>
                <p className="text-red-700 text-xs">
                  Place an order for ₹500 or more and get 15% off on the total.
          ...
          <div className="mb-3">
            ...
            >
              ...
              <span>
                {claiming ? 'Redirecting...' : 'Order Now'}
              </span>
            </button>
          </div>
